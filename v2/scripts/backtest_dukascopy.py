"""Backtest profesional usando 2 anos de datos Dukascopy.

Flujo identico a backtest_twelvedata.py pero usando DukascopyExtractor:
1. Descargar OHLC del periodo solicitado (Dukascopy, sin rate limits, con cache)
2. Generar features (pipeline validado — NO modificado)
3. Aplicar estrategia v3 (NO modificada)
4. Ejecutar backtest con engine validado (NO modificado)
5. Calcular metricas profesionales
6. Generar graficos
7. Guardar resultados

Ventajas sobre Twelve Data:
- Sin limite de 5000 velas por request
- Sin API key necesaria
- Sin rate limit (sin sleep entre descargas)
- Tick volume real disponible
- Hasta 20 anos de historia

Estimacion de recursos para 2 anos (2024-06-01 a 2026-05-28):
- M1: ~735,000 velas, ~60 MB OHLCV
- Snapshots con 163 features: ~958 MB RAM
- Pico durante feature engineering: ~2-3 GB RAM
- Tiempo estimado feature engineering: 8-15 min (primera ejecucion)
- Segunda ejecucion (desde cache): descarga en segundos, features en el mismo tiempo

Uso:
    python -m v2.scripts.backtest_dukascopy --start 2024-06-01 --end 2026-05-28

    # Periodo mas corto para prueba rapida (1 mes)
    python -m v2.scripts.backtest_dukascopy --start 2025-01-01 --end 2025-01-31

    # Sin graficos (mas rapido)
    python -m v2.scripts.backtest_dukascopy --start 2024-06-01 --end 2026-05-28 --no-plots

    # Forzar re-descarga ignorando cache
    python -m v2.scripts.backtest_dukascopy --start 2024-06-01 --end 2026-05-28 --force-refresh
"""
import argparse
import logging
import sys
...
from v2.src.data.dukascopy_to_features import (
    build_snapshots_from_dukascopy,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Backtest de la estrategia v3 usando datos de Dukascopy (2 anos)"
    )
    p.add_argument("--symbol",     default="XAUUSD",      help="Simbolo (default: XAUUSD)")
    p.add_argument("--start",      default="2024-06-01",   help="Fecha inicio YYYY-MM-DD")
    p.add_argument("--end",        default="2026-05-28",   help="Fecha fin YYYY-MM-DD")
    ...
    p.add_argument("--slip-entry", type=float, default=0.5,   help="Slippage entry pips (default: 0.5)")
    p.add_argument("--slip-exit",  type=float, default=0.5,   help="Slippage exit pips (default: 0.5)")
...
def main() -> None:
    ...
    print("=" * 70)
    print(f"   BACKTEST Dukascopy — {args.symbol}")
    ...
    print(f"   AVISO   : Feature engineering de 2 anos requiere ~2-3 GB RAM y 8-15 min")
    print("=" * 70)
    ...
    if m1.empty:
        logger.error("Sin datos M1. Verifica que dukascopy-python esta instalado: pip install dukascopy-python")
        sys.exit(1)

    # -- 2. Generar features -----------------------------------------------------
    logger.info("=== FEATURE ENGINEERING (puede tardar varios minutos) ===")
    safe_symbol = args.symbol.replace("/", "_")
    out_dir = (
        Path(args.output_dir)
        if args.output_dir
        else Path(f"v2/data/backtest_dukascopy/{safe_symbol}_{args.start}_{args.end}")
    )
    snap_path = out_dir / "snapshots_M1.parquet"

    snapshots = build_snapshots_from_dukascopy(ohlc, save_path=snap_path)
    ...
    if signals.empty:
        logger.warning("Sin senales en el periodo. Considera ampliar el rango de fechas.")
        sys.exit(0)
