"""Backtest de 3 anos (2023-05-29 -> 2026-05-28) usando Twelve Data (plan GROW).

Identico en estructura a backtest_3years.py (Dukascopy), pero usando
TwelveDataExtractor con paginacion automatica y el rate limit del plan GROW
(55 req/min). Reusa el pipeline validado (estrategia v3, features, engine,
metricas) SIN modificarlo.

Recursos esperados (ver analisis Fase 8):
- ~1.1M velas M1 (3 anos)
- Snapshots finales: ~1.4 GB en RAM
- Pico durante build_snapshots (merge_asof + concat de bloques): ~4-5 GB
- Tiempo total estimado: descarga mas lenta que Dukascopy (paginacion via API),
  features + backtest igual que backtest_3years.py
...
Uso:
    python -m v2.scripts.backtest_3years_twelvedata
"""
import gc
import logging
import sys
...
def main() -> None:
    ...
    print("=" * 70)
    print("   BACKTEST 3 ANOS -- XAUUSD Asian Momentum v3 (Twelve Data)")
    print(f"   Periodo : {start.date()} -> {end.date()}")
    print("   Fuente  : Twelve Data (plan GROW, 55 req/min, paginacion automatica)")
    ...
    out_dir = Path(f"v2/data/backtest_3years_twelvedata/XAU_USD_{start.date()}_{end.date()}")
    ...
    generate_full_report(
        ...
        title_prefix=f"v3 XAUUSD {start.date()}/{end.date()} (3 anos, Twelve Data)",
    )
