"""Backtest de 5 anos (2021-05-29 -> 2026-05-28) con reporte mensual detallado.
...
- ~1.8M velas M1
- Snapshots finales: ~2.35 GB en RAM
- Pico durante build_snapshots (merge_asof + concat de bloques): ~6-8 GB
- Tiempo total estimado: 1-2 horas (descarga + features + backtest)

Recomendaciones antes de lanzar:
- Cerrar Chrome / IDE pesados para liberar RAM
- Verificar que hay >8 GB libres (Administrador de tareas)
- No interrumpir el proceso durante build_snapshots (es la fase critica de RAM)

Uso:
    python -m v2.scripts.backtest_5years
"""
import gc
import logging
import sys
...
def main() -> None:
    ...
    logger.info("=== FEATURE ENGINEERING (15-30 min para 5 anos, fase critica de RAM) ===")
    ...
    out_dir = Path(f"v2/data/backtest_5years/XAUUSD_{start.date()}_{end.date()}")
    ...
    generate_full_report(
        ...
        title_prefix=f"v3 XAUUSD {start.date()}/{end.date()} (5 anos)",
    )
