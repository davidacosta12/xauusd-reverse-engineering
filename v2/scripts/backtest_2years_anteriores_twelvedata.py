"""Backtest de 2 anos anteriores (2021-05-29 -> 2023-05-28) usando Twelve Data (plan GROW).

Bloque complementario a backtest_3years_twelvedata.py: junto con ese backtest
(2023-05-29 -> 2026-05-28), cubre los 5 anos completos del periodo de analisis.

Identico en estructura a backtest_3years_twelvedata.py, solo cambia el rango
de fechas y la carpeta de salida. Reusa el pipeline validado (estrategia v3,
features, engine, metricas) SIN modificarlo.

Recursos esperados:
- ~1.1M velas M1 (estimado, 2 anos suelen tener algo menos que 3)
- Pico durante build_snapshots (merge_asof + concat de bloques): ~4-5 GB
- Tiempo total estimado: 10-15 min (menos datos que el bloque de 3 anos)

Recomendaciones antes de lanzar:
- Cerrar Chrome / IDE pesados para liberar RAM
- Verificar que hay >4 GB libres (Administrador de tareas)
- No interrumpir el proceso durante build_snapshots (es la fase critica de RAM)

Uso:
    python -m v2.scripts.backtest_2years_anteriores_twelvedata
"""
import gc
import logging
import sys
...
from v2.src.data.twelve_data_to_features import fetch_all_timeframes
...
RATE_LIMIT_SLEEP_SEC_GROW = 1.5  # plan GROW: 55 req/min


def main() -> None:
    start = pd.Timestamp("2021-05-29", tz="UTC")
    end   = pd.Timestamp("2023-05-28 23:59:59", tz="UTC")

    print("=" * 70)
    print("   BACKTEST 2 ANOS ANTERIORES -- XAUUSD Asian Momentum v3")
    print(f"   Periodo : {start.date()} -> {end.date()}")
    print("   Fuente  : Twelve Data plan GROW")
    print("   PROPOSITO: completar 5 anos junto con el backtest 3 anos ya hecho")
    print("   ESTIMADO: ~4-5 GB RAM pico durante feature engineering")
    print("   ESTRATEGIA: SIN modificar (defaults validados v3)")
    print("=" * 70)

    # -- 1. Descarga (TwelveDataExtractor con paginacion + cache local) ----------
    logger.info("=== DESCARGA (con cache local, plan GROW) ===")
    extractor = TwelveDataExtractor(rate_limit_sleep_sec=RATE_LIMIT_SLEEP_SEC_GROW)
    ohlc = fetch_all_timeframes(
        extractor, "XAU/USD", start.to_pydatetime(), end.to_pydatetime(),
        sleep_between=RATE_LIMIT_SLEEP_SEC_GROW,
    )
    ...
    if m1.empty:
        logger.error("Sin datos M1. Verifica TWELVE_DATA_API_KEY en .env.")
        sys.exit(1)

    # -- 2. Feature engineering (pipeline validado, pasada unica) ----------------
    logger.info("=== FEATURE ENGINEERING (10-20 min para 2 anos, fase critica de RAM) ===")
    ...
    out_dir = Path(f"v2/data/backtest_2years_anteriores_twelvedata/XAU_USD_{start.date()}_{end.date()}")
    ...
    generate_full_report(
        ...
        title_prefix=f"v3 XAUUSD {start.date()}/{end.date()} (2 anos anteriores, Twelve Data)",
    )
